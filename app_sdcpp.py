...
SDCPP_BIN = os.getenv("SDCPP_BIN", "/app/stable-diffusion.cpp/bin/sd")
...
def run_generate(prompt: str, negative: str, seed_mode: str, seed: int, steps: int, cfg: float, width: int, height: int):
    if not prompt.strip():
    ...
    if not Path(DIFFUSION_MODEL).exists():
        raise gr.Error(f"GGUF Model nicht gefunden: {DIFFUSION_MODEL}")
    ...
    return str(out_file), final_seed, " ".join(shlex.quote(c) for c in cmd)


with gr.Blocks(title="Flux2 GGUF via stable-diffusion.cpp", theme=gr.themes.Soft()) as demo:
    gr.Markdown("# FLUX.2 klein 4B (GGUF) via stable-diffusion.cpp")
    ...
    cmdline = gr.Textbox(label="Ausgeführter sd.cpp Command", lines=2)

    run.click(run_generate, inputs=[prompt, negative, seed_mode, seed, steps, cfg, width, height], outputs=[image, used_seed, cmdline])
